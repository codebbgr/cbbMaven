...
/**
 * The <code>&lt;parent&gt;</code> element contains information required to locate the parent
 * project from which this project will inherit from. <strong>Note:</strong> The children of this
 * element are not interpolated and must be given as literal values.
 *
...
public class Parent
  ...
  /**
   * The relative path of the parent <code>pom.xml</code> file within the check out. If not
   * specified, it defaults to <code>../pom.xml</code>. Maven looks for the parent POM first in this
   * location on the filesystem, then the local repository, and lastly in the remote repo. <code>
   * relativePath</code> allows you to select a different location, for example when your structure
   * is flat, or deeper without an intermediate parent POM. However, the group ID, artifact ID and
   * version are still required, and must match the file in the location given or it will revert to
   * the repository for the POM. This feature is only for enhancing the development in a local
   * checkout of that project. Set the value to an empty string in case you want to disable the
  ...
  /**
   * Get the relative path of the parent <code>pom.xml</code> file within the check out. If not
   * specified, it defaults to <code>../pom.xml</code>. Maven looks for the parent POM first in this
   * location on the filesystem, then the local repository, and lastly in the remote repo. <code>
   * relativePath</code> allows you to select a different location, for example when your structure
   * is flat, or deeper without an intermediate parent POM. However, the group ID, artifact ID and
  ...
  /**
   * Set the relative path of the parent <code>pom.xml</code> file within the check out. If not
   * specified, it defaults to <code>../pom.xml</code>. Maven looks for the parent POM first in this
   * location on the filesystem, then the local repository, and lastly in the remote repo. <code>
   * relativePath</code> allows you to select a different location, for example when your structure
   * is flat, or deeper without an intermediate parent POM. However, the group ID, artifact ID and
