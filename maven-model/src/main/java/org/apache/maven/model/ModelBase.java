...
public class ModelBase
  ...
  /**
   * Distribution information for a project that enables deployment of the site and artifacts to
   * remote web servers and repositories respectively.
   */
  ...
  /**
   * Default dependency information for projects that inherit from this one. The dependencies in
   * this section are not immediately resolved. Instead, when a POM derived from this one declares a
   * dependency described by a matching groupId and artifactId, the version and other values from
   * this section are used for that dependency if they were not already specified.
   */
   ...
   * Maven-generated site. These reports will be run when a user executes <code>mvn site</code>. All
   * of the reports will be included in the navigation bar for browsing.
   */
      ...
      if (this.reports != null) {
        copy.reports =
            new org.codehaus.plexus.util.xml.Xpp3Dom(
                (org.codehaus.plexus.util.xml.Xpp3Dom) this.reports);
      }
  ...
  /**
   * Get default dependency information for projects that inherit from this one. The dependencies in
   * this section are not immediately resolved. Instead, when a POM derived from this one declares a
   * dependency described by a matching groupId and artifactId, the version and other values from
   * this section are used for that dependency if they were not already specified.
   *
  ...
  /**
   * Get distribution information for a project that enables deployment of the site and artifacts to
   * remote web servers and repositories respectively.
   *
  ...
  /**
   * Get this element includes the specification of report plugins to use to generate the reports on
   * the Maven-generated site. These reports will be run when a user executes <code>mvn site</code>.
  ...
  /**
   * Set this element describes all of the dependencies associated with a project. These
   * dependencies are used to construct a classpath for your project during the build process. They
   * are automatically downloaded from the repositories defined in this project. See <a
  ...
  /**
   * Set default dependency information for projects that inherit from this one. The dependencies in
   * this section are not immediately resolved. Instead, when a POM derived from this one declares a
   * dependency described by a matching groupId and artifactId, the version and other values from
   * this section are used for that dependency if they were not already specified.
   *
  ...
  /**
   * Set distribution information for a project that enables deployment of the site and artifacts to
   * remote web servers and repositories respectively.
   *
  ...
  /**
   * Set the modules (sometimes called subprojects) to build as a part of this project. Each module
   * listed is a relative path to the directory containing the module. To be consistent with the way
   * default urls are calculated from parent, it is recommended to have module names match artifact
   * ids.
   *
   * @param modules
   */
  ...
  /**
   * Set properties that can be used throughout the POM as a substitution, and are used as filters
  ...
  /**
   * Set this element includes the specification of report plugins to use to generate the reports on
   * the Maven-generated site. These reports will be run when a user executes <code>mvn site</code>.
   * All of the reports will be included in the navigation bar for browsing.
   *
   * @param reporting
