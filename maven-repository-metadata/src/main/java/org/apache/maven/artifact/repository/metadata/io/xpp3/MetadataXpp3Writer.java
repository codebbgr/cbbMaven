...
public class MetadataXpp3Writer {
  ...
  private void writeSnapshot(Snapshot snapshot, String tagName, XmlSerializer serializer)
    ...
    if (snapshot.getTimestamp() != null) {
      serializer
          .startTag(NAMESPACE, "timestamp")
          .text(snapshot.getTimestamp())
          .endTag(NAMESPACE, "timestamp");
    }
    if (snapshot.getBuildNumber() != 0) {
      serializer
          .startTag(NAMESPACE, "buildNumber")
          .text(String.valueOf(snapshot.getBuildNumber()))
          .endTag(NAMESPACE, "buildNumber");
    }
    if (snapshot.isLocalCopy() != false) {
      serializer
          .startTag(NAMESPACE, "localCopy")
          .text(String.valueOf(snapshot.isLocalCopy()))
          .endTag(NAMESPACE, "localCopy");
    }
    serializer.endTag(NAMESPACE, tagName);
   ...
   * @throws java.io.IOException
   */
  private void writeSnapshotVersion(
      SnapshotVersion snapshotVersion, String tagName, XmlSerializer serializer)
      throws java.io.IOException {
    serializer.startTag(NAMESPACE, tagName);
    if ((snapshotVersion.getClassifier() != null) && !snapshotVersion.getClassifier().equals("")) {
      serializer
          .startTag(NAMESPACE, "classifier")
          .text(snapshotVersion.getClassifier())
          .endTag(NAMESPACE, "classifier");
    }
    if (snapshotVersion.getExtension() != null) {
      serializer
          .startTag(NAMESPACE, "extension")
          .text(snapshotVersion.getExtension())
          .endTag(NAMESPACE, "extension");
    }
    if (snapshotVersion.getVersion() != null) {
      serializer
          .startTag(NAMESPACE, "value")
          .text(snapshotVersion.getVersion())
          .endTag(NAMESPACE, "value");
    }
    if (snapshotVersion.getUpdated() != null) {
      serializer
          .startTag(NAMESPACE, "updated")
          .text(snapshotVersion.getUpdated())
          .endTag(NAMESPACE, "updated");
    }
    serializer.endTag(NAMESPACE, tagName);
  ...
  private void writeVersioning(Versioning versioning, String tagName, XmlSerializer serializer)
    ...
    if (versioning.getLatest() != null) {
      serializer
          .startTag(NAMESPACE, "latest")
          .text(versioning.getLatest())
          .endTag(NAMESPACE, "latest");
    }
    if (versioning.getRelease() != null) {
      serializer
          .startTag(NAMESPACE, "release")
          .text(versioning.getRelease())
          .endTag(NAMESPACE, "release");
    }
    ...
    if (versioning.getLastUpdated() != null) {
      serializer
          .startTag(NAMESPACE, "lastUpdated")
          .text(versioning.getLastUpdated())
          .endTag(NAMESPACE, "lastUpdated");
    }
    if ((versioning.getSnapshotVersions() != null)
        && (versioning.getSnapshotVersions().size() > 0)) {
