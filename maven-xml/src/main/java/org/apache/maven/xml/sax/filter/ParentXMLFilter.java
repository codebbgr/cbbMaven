...
/**
 * Transforms relativePath to version. We could decide to simply allow {@code <parent/>}, but let's
 * require the GA for now for checking This filter does NOT remove the relativePath (which is done
 * by {@link RelativePathXMLFilter}, it will only optionally include the version based on the path
 *
        ...
        case "parent":
          ...
          if (!hasVersion && resolvedParent.isPresent()) {
            try (Includer i = super.include()) {
              ...
              super.characters(
                  resolvedParentVersion.toCharArray(), 0, resolvedParentVersion.length());
