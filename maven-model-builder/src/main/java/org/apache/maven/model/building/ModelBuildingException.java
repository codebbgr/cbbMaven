...
/**
 * Signals one ore more errors during model building. The model builder tries to collect as many
 * problems as possible before eventually failing to provide callers with rich error information.
 * Use {@link #getProblems()} to query the details of the failure.
 *
...
public class ModelBuildingException extends Exception {
  ...
  /**
   * Gets the identifier of the POM whose effective model could not be built. The general format of
   * the identifier is {@code <groupId>:<artifactId>:<version>} but some of these coordinates may
   * still be unknown at the point the exception is thrown so this information is merely meant to
   * assist the user.
   *
